...
public class GmRemove implements ICommand {
    @Override
    ...
    public void execute(MinecraftServer server, ICommandSender sender, String[] args) throws CommandException {
        if(args.length != 1) {
            sender.sendMessage(new TextComponentString("Incorrect usage! Correct use /gmremove <name>"));
            return;
        }
        ...
        if(!allowedPlayers.contains(player)) {
            sender.sendMessage(new TextComponentString("Player not in list"));
        } else {
            ...
            sender.sendMessage(new TextComponentString(player + " removed from list"));
        }
    }
    ...
    public List<String> getTabCompletions(MinecraftServer server, ICommandSender sender, String[] args, @Nullable BlockPos targetPos) {
        ...
        for(String player : OreCraft.config.getStringList("gm-allowed-players")) {
